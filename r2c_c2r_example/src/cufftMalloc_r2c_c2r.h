...
#ifdef USE_DOUBLE
        CUDA_RT_CALL( cufftExecD2Z( fft_forward, const_cast<T *>( inputSignal ), d_bufferData ) );
        CUDA_RT_CALL( cufftExecZ2D( fft_inverse, d_bufferData, outputData ) );

#else
        CUDA_RT_CALL( cufftExecR2C( fft_forward, const_cast<T *>( inputSignal ), d_bufferData ) );
        CUDA_RT_CALL( cufftExecC2R( fft_inverse, d_bufferData, outputData ) );
#endif
    }
